...
async function connectOkxWallet(updateOkxWallet) {
	if (typeof window.okxwallet !== 'undefined') {
		try {
			await window.okxwallet.bitcoin.connect();
			updateOkxWallet(true);
			location.reload();
		} catch (e) {
			console.log(e);
			updateOkxWallet(false);
		}
	} else {
		alert('OKX Wallet Not Connected');
	}
}
...
export { connectWallet, connectOkxWallet };
